...
[ "${CH_CORE_PREFLIGHT_QUIESCED:-}" = 'YES' ] ||
  die 'Set CH_CORE_PREFLIGHT_QUIESCED=YES only after every client is closed.'
[ "${CH_CORE_PREFLIGHT_WINDOWS_OUTBOX:-}" = '0' ] ||
  die 'Windows outbox must be measured and equal to 0.'
[ "${CH_CORE_PREFLIGHT_ANDROID_OUTBOX:-}" = '0' ] ||
  die 'Android outbox must be measured and equal to 0.'
...
bundle_name="chu-v021-${stamp}.bundle"
bundle_path="/backup/$bundle_name"
receipt_name="chu-v021-${stamp}.preflight.txt"
...
docker compose --profile ops run --rm ch-core-ops \
  /bin/sh -eu -c '
    ...
    case "$receipt_name" in
      chu-v021-[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]-[0-9][0-9][0-9][0-9][0-9][0-9].preflight.txt) ;;
    ...
    {
      printf "CH_CORE_V021_PREFLIGHT_V1\n"
      printf "TIME_WITA=%s\n" "$time_wita"
      printf "OUTBOX_WINDOWS=0\n"
      printf "OUTBOX_ANDROID=0\n"
      printf "QUIESCED=YES\n"
  ...
  ' ch-core-v021-counts "$receipt_name" "$time_wita" "$required_tables" "$expected_migrations"
...
docker compose --profile ops run --rm ch-core-ops \
  ...
  ' ch-core-v021-finalize "$receipt_name" "$bundle_name"
