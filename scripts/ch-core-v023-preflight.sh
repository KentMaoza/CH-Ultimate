...
bundle_name="chu-v023-${stamp}.bundle"
bundle_path="/backup/$bundle_name"
receipt_name="chu-v023-${stamp}.preflight.txt"
...
docker compose --profile ops run --rm ch-core-ops \
  /bin/sh -eu -c '
    ...
    case "$receipt_name" in
      chu-v023-[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]-[0-9][0-9][0-9][0-9][0-9][0-9].preflight.txt) ;;
    ...
    {
      printf "CH_CORE_V023_PREFLIGHT_V1\n"
      printf "TIME_WITA=%s\n" "$time_wita"
  ...
  ' ch-core-v023-counts "$receipt_name" "$time_wita" "$required_tables" "$expected_migrations" \
...
docker compose --profile ops run --rm ch-core-ops \
  ...
  ' ch-core-v023-finalize "$receipt_name" "$bundle_name"
