...
const RELEASE_NOTES = 'docs/releases/pilot-0.2.5.md';
...
function stagePilotDraft({
  ...
  const releaseMatch = /^pilot-v0\.2\.5-r([2-9]|[1-9][0-9])$/.exec(releaseTag || '');
  if (!releaseMatch) throw new Error('CHU_PILOT_RELEASE_TAG is invalid.');
  const candidateNumber = Number(releaseMatch[1]);
  const releaseTitle = `CH Ultimate pilot v0.2.5 r${releaseMatch[1]}`;
  ...
  if (candidateNumber > 2) {
    const previousTag = `pilot-v0.2.5-r${candidateNumber - 1}`;
