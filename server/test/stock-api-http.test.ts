...
function harness() {
  ...
  const stockChecks = {
    checkOnline: vi.fn(async () => ({ apiSchemaVersion: 2 })),
    checkOffline: vi.fn(async () => ({ apiSchemaVersion: 2 })),
  };
  const packageBarcodes = {
    register: vi.fn(async () => ({ apiSchemaVersion: 2 })),
    remove: vi.fn(async () => ({ apiSchemaVersion: 2 })),
    reassign: vi.fn(async () => ({ apiSchemaVersion: 2 })),
  };
  const app = buildApp({
