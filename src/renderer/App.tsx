...
const modules: Array<{ id: ModuleId; label: string; icon: NavIconName }> = [
  ...
  { id: 'dataExport', label: 'Ekspor Data', icon: 'export' },
...
function NavIcon({ name }: { name: NavIconName }) {
  ...
  return <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false" {...common}>
    ...
    {name === 'export' && <><path d="M5 3h10l4 4v14H5Z" /><path d="M15 3v5h4M12 11v7M9 15l3 3 3-3" /></>}
  ...
  return (
    <div className={`app-shell${collapsed ? ' rail-collapsed' : ''}`}>
      ...
      <main className="app-main">
        ...
        {active === 'inventory' ? <InventoryPage /> : active === 'stockCheck' ? <StockCheckView gateway={gateway} mode="desktop" /> : active === 'skuChanges' ? <SkuChangesPage coreBacked={coreBacked} /> : active === 'shareRecommendations' ? <ShareRecommendationsPage coreBacked={coreBacked} /> : active === 'dataExport' ? <OperationalExportPage /> : active === 'create' ? <CreateSkuPage coreBacked={coreBacked} /> : active === 'label' ? <LabelPage coreBacked={coreBacked} /> : active === 'notaArchive' ? <ArchiveNotaPage view={archiveView} onViewChange={setArchiveView} onOpenNota={openNota} /> : active === 'revenue' ? <RevenuePage coreBacked={coreBacked} onOpenSettings={() => setActive('settings')} /> : active === 'empty' ? <EmptyStockPage coreBacked={coreBacked} /> : active === 'settings' ? <SettingsPage coreBacked={coreBacked} /> : (
...
export function App({
  gateway,
  coreBacked = false,
  outputBridge = window.chOutput,
}: {
  gateway: OperationsGateway;
  coreBacked?: boolean;
  outputBridge?: ChOutputBridge;
}) {
  return <OperationsProvider gateway={gateway}><OutputProvider bridge={outputBridge}><RevenueAccessProvider><AppLayout gateway={gateway} coreBacked={coreBacked} /></RevenueAccessProvider></OutputProvider></OperationsProvider>;
}
