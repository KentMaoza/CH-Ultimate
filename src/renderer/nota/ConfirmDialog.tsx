...
export function ConfirmDialog({ open, title, children, onCancel, onConfirm, restoreFocusTo }: {
  open: boolean;
  title: string;
  children: ReactNode;
  onCancel: () => void;
  ...
  return <div className="chu-nota-workspace__dialog-backdrop" onMouseDown={backdrop}>
    <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="chu-nota-confirm-title" className="chu-nota-workspace__dialog" onKeyDown={keyDown}>
      ...
      <p>{children}</p>
      <div><button data-dialog-initial-focus onClick={close}>Batal</button><button className="chu-nota-workspace__complete" onClick={onConfirm}>Selesaikan</button></div>
    </div>
  </div>;
}
