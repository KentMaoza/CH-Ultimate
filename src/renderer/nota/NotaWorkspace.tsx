  ...
  return <main className="chu-nota-workspace" data-testid="chu-nota-workspace" aria-busy={busy || undefined} style={{ '--nota-font-scale': fontScale / 100 } as CSSProperties}>
    ...
    {selectedTransaction && page ? <>
      ...
      <section className="chu-nota-workspace__page-tabs" aria-label="Halaman aktif">
        {selectedTransaction.pages.filter((item) => item.status === 'active').map((item) => <button disabled={busy} key={item.id} aria-label={`Halaman ${item.suffix}`} aria-pressed={item.id === page.id} onClick={() => selectPage({ transactionId: selectedTransaction.id, pageId: item.id })}>Nota {item.suffix}</button>)}
