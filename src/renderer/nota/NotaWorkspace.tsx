...
import { capitalizeSentenceStarts, formatRupiah } from '../format';
...
import { createNotaUndoHistory } from './nota-undo-history';
...
export function NotaWorkspace({ onBack, initialSelection }: { onBack: () => void; initialSelection?: Selection }) {
  ...
  const [lifecycleUndo, setLifecycleUndo] = useState<{ label: string; action: () => Promise<void> } | null>(null);
  ...
  const lifecycleUndoTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  const editHistory = useRef(createNotaUndoHistory());
  ...
  useEffect(() => () => { if (lifecycleUndoTimer.current) clearTimeout(lifecycleUndoTimer.current); }, []);
  useEffect(() => {
  ...
  useEffect(() => {
    const shortcut = (event: KeyboardEvent) => {
      ...
      if ((event.ctrlKey || event.metaKey) && !event.shiftKey && event.key.toLowerCase() === 'z' && !(event.target instanceof Element && event.target.closest('[role="dialog"]'))) {
        event.preventDefault();
        undoEditRef.current();
      }
    };
  ...
  const showLifecycleUndo = (label: string, action: () => Promise<void>) => {
    if (lifecycleUndoTimer.current) clearTimeout(lifecycleUndoTimer.current);
    setLifecycleUndo({ label, action });
    lifecycleUndoTimer.current = setTimeout(() => setLifecycleUndo(null), 10_000);
  };
  const restoreLifecycleUndo = async () => {
    if (!lifecycleUndo) return;
    const result = await run(lifecycleUndo.action, 'Pembatalan tidak dapat dipulihkan.');
    if (!result.ok) return;
    if (lifecycleUndoTimer.current) clearTimeout(lifecycleUndoTimer.current);
    setLifecycleUndo(null);
  };
  const cancelPage = async (choice: Selection) => {
    ...
    showLifecycleUndo(`Halaman ${pageToCancel.suffix} dipindahkan ke Sampah.`, () => gateway.restoreNotaPage(choice.transactionId, choice.pageId));
  };
  ...
  const complete = async (transactionId: string) => {
    ...
    editHistory.current.clear(transactionId);
    setHistoryVersion((value) => value + 1);
    return true;
  };
  const recordEdit = (transactionId: string, key: string, action: () => Promise<void>) => {
    editHistory.current.push({ transactionId, key, createdAt: Date.now(), undo: action });
    setHistoryVersion((value) => value + 1);
  };
  const restoreEdit = async () => {
    if (!selectedTransaction) return;
    const entry = editHistory.current.pop(selectedTransaction.id);
    if (!entry) return;
    const result = await run(entry.undo, 'Perubahan tidak dapat di-undo.');
    if (!result.ok) editHistory.current.push({ ...entry, createdAt: Date.now() });
    setHistoryVersion((value) => value + 1);
  };
  undoEditRef.current = () => { void restoreEdit(); };
  const updateTransaction = (transactionId: string, patch: TransactionPatch) => {
    ...
    const before = Object.fromEntries(keys.map((key) => [key, transaction[key]])) as TransactionPatch;
    if (!keys.some((key) => before[key] !== patch[key])) return;
    recordEdit(transactionId, `transaction:${keys.join(',')}`, () => gateway.updateNotaTransaction(transactionId, before));
  ...
  const updateLine = (transactionId: string, pageId: string, lineId: string, patch: Partial<NotaLine>) => {
    ...
    const before = Object.fromEntries(keys.map((key) => [key, line[key]])) as Partial<NotaLine>;
    if (!keys.some((key) => before[key] !== patch[key])) return;
    recordEdit(transactionId, `line:${pageId}:${lineId}:${keys.join(',')}`, () => gateway.updateNotaLine(transactionId, pageId, lineId, before));
  ...
  const deleteLine = (transactionId: string, pageId: string, lineId: string) => {
    const line = gateway.getSnapshot().notaTransactions.find((item) => item.id === transactionId)?.pages.find((item) => item.id === pageId)?.lines.find((item) => item.id === lineId);
    if (!line) return;
    recordEdit(transactionId, `line:${pageId}:${lineId}:clear`, () => gateway.updateNotaLine(transactionId, pageId, lineId, line));
  ...
  const confirmAction = async () => {
    ...
    if (pending.kind === 'cancel') {
      ...
      if (result.ok && cancelled?.status === 'cancelled') {
        editHistory.current.clear(pending.transactionId);
        setHistoryVersion((value) => value + 1);
        showLifecycleUndo('Transaksi dipindahkan ke Sampah.', () => gateway.restoreNotaTransaction(pending.transactionId));
  ...
  const canUndoEdit = Boolean(editable && selectedTransaction && editHistory.current.has(selectedTransaction.id));
  ...
  return <main className="chu-nota-workspace" data-testid="chu-nota-workspace" aria-busy={busy || undefined} style={{ '--nota-font-scale': fontScale / 100 } as CSSProperties}>
    <header className="chu-nota-workspace__toolbar">
      ...
      <button aria-label="Undo perubahan" disabled={!canUndoEdit || busy} title="Undo perubahan terakhir (Ctrl/Cmd+Z)" onClick={() => void restoreEdit()}>Undo</button>
    ...
    {lifecycleUndo && <p className="chu-nota-workspace__notice" role="status">{lifecycleUndo.label} <button disabled={busy} onClick={() => void restoreLifecycleUndo()}>Urungkan</button></p>}
