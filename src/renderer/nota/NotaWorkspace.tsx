...
export function NotaWorkspace({ onBack }: { onBack: () => void }) {
  ...
  return <main className="chu-nota-workspace" data-testid="chu-nota-workspace" style={{ zoom: zoom / 100 }}>
    ...
    {drawer === 'working' && <WorkingDrawer transactions={state.notaTransactions} selected={selected} onClose={() => setDrawer(null)} onSelect={choose} onAdd={(id) => void addPage(id)} onCancelPage={(choice) => void cancelPage(choice)} onCancelTransaction={(id) => requestConfirm('cancel', id, drawerRestoreFocus)} restoreFocusTo={drawerRestoreFocus} busy={busy} />}
    {drawer === 'list' && <ListDrawer transactions={state.notaTransactions} initialTab={listTab} onTabChange={setListTab} onClose={() => setDrawer(null)} onOpenArchive={(id, pageId) => requestConfirm('reopen', id, drawerRestoreFocus, pageId)} onRestoreTransaction={(id) => void restoreTransaction(id)} onRestorePage={(choice) => void restorePage(choice)} restoreFocusTo={drawerRestoreFocus} busy={busy} />}
