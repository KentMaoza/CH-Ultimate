...
export function ArchiveNotaPage({ view, onViewChange, onOpenNota }: {
  ...
  return <div className={`feature-page archive-nota${showPreview ? '' : ' archive-nota--preview-collapsed'}`}>
    <section className="archive-nota__filters card">
      ...
      {!result.items.length && <p className="empty-state">{view.tab === 'archive' ? 'Arsip belum memiliki nota.' : view.tab === 'finished' ? 'Belum ada nota yang menunggu pengiriman.' : 'Sampah kosong.'}</p>}
    ...
    {showPreview && <section id="archive-nota-preview" className="archive-nota__preview" aria-label={view.tab === 'finished' ? 'Preview selesai nota' : 'Preview arsip nota'}>{selectedTransaction && selectedPage ? <><header><div><span>{view.tab === 'finished' ? 'SELESAI · BARANG DIKIRIM NANTI' : 'ARSIP · HANYA LIHAT'}</span><strong className="archive-nota__preview-customer">{selectedTransaction.customerName || 'Tanpa pelanggan'}</strong><b className="archive-nota__preview-place">{selectedTransaction.customerPlace || 'Tanpa tempat'}</b><small className="archive-nota__preview-number">{selectedTransaction.baseNumber}{selectedPage.suffix}</small><small className="archive-nota__preview-date">{selectedTransaction.transactionDate}</small></div><button className="button primary" onClick={() => setReopen(selectedTransaction)}>Buka kembali untuk edit</button></header><div className="archive-nota__page-total">Total halaman <strong>{formatRupiah(selectedPage.lines.reduce((sum, line) => sum + lineTotal(line), 0))}</strong></div><div className="chu-nota-workspace"><NotaGrid lines={selectedPage.lines} suffix={selectedPage.suffix} skus={state.skus} editable={false} busy={false} invalidValues={{}} onInvalidChange={() => {}} onUpdate={() => {}} onDelete={() => {}} /></div></> : <p className="empty-state">Pilih transaksi untuk melihat preview.</p>}</section>}
