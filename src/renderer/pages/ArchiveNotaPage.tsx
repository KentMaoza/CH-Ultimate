...
export function ArchiveNotaPage({ view, onViewChange, onOpenNota }: {
  ...
  return <div className="feature-page archive-nota">
    ...
    <section className="archive-nota__preview" aria-label="Preview arsip nota">{view.tab === 'archive' && selectedTransaction && selectedPage ? <><header><div><span>SELESAI · HANYA LIHAT</span><strong className="archive-nota__preview-customer">{selectedTransaction.customerName || 'Tanpa pelanggan'}</strong><b className="archive-nota__preview-place">{selectedTransaction.customerPlace || 'Tanpa tempat'}</b><small className="archive-nota__preview-number">{selectedTransaction.baseNumber}{selectedPage.suffix}</small><small className="archive-nota__preview-date">{selectedTransaction.transactionDate}</small></div><button className="button primary" onClick={() => setReopen(selectedTransaction)}>Buka kembali untuk edit</button></header><div className="archive-nota__page-total">Total halaman <strong>{formatRupiah(selectedPage.lines.reduce((sum, line) => sum + lineTotal(line), 0))}</strong></div><div className="chu-nota-workspace"><NotaGrid lines={selectedPage.lines} suffix={selectedPage.suffix} skus={state.skus} editable={false} busy={false} invalidValues={{}} onInvalidChange={() => {}} onUpdate={() => {}} onDelete={() => {}} /></div></> : <p className="empty-state">Pilih transaksi untuk melihat preview.</p>}</section>
