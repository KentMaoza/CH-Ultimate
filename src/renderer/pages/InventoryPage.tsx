...
import { buildBarcodeDocumentPlan, skuProductCode } from '../../domain/output-documents';
...
export function InventoryPage() {
  ...
  function openAdjustment(sku: Sku, direction: 1 | -1) {
    setAdjusting({ sku, direction });
    setQuantity('');
    setAdjustmentReason('');
  }
  ...
  const printingProductCode = printing ? skuProductCode(printing) : '';
  const canImportCatalogue =
    gateway.capabilities.canImportInitialCatalogue ||
    gateway.capabilities.canStageInitialCatalogue;
  const requestBarcodeOutput = async (action: 'print' | 'pdf') => {
    if (!printing || !validPrintQuantity) return;
    setMessage('');
    try {
