...
import { OwnerPairingCard } from '../components/OwnerPairingCard';
...
export function SettingsPage({ coreBacked = false }: { coreBacked?: boolean }) {
  ...
  const ownerBridge = coreBacked ? window.chCore : undefined;
  ...
  return <div className="feature-page settings-grid">
    ...
    <section className="settings-card dark-card"><span>APLIKASI</span><h2>CH Ultimate 0.2.6</h2><p>{coreBacked ? 'Electron · Node API · MariaDB · Bahasa Indonesia · IDR · WITA' : 'Electron frontend-only · Bahasa Indonesia · IDR · WITA'}</p><hr /><p>{coreBacked ? 'Data bisnis disinkronkan melalui CH Core pada NAS lokal. Auto-update belum tersedia. Cetak dan Simpan PDF memakai dialog sistem.' : 'NAS, database, auto-update, dan mobile Core belum tersedia. Cetak dan Simpan PDF memakai dialog sistem.'}</p></section>
    {ownerBridge ? <OwnerPairingCard bridge={ownerBridge} /> : null}
