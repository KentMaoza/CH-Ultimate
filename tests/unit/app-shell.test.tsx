...
test('shows all operational modules and switches the active page', () => {
  render(<App />);
  for (const label of ['SKU Gudang', 'Perubahan SKU', 'Buat SKU', 'Template Label & Invoice', 'Nota', 'Arsip Nota', 'Laporan Omzet', 'Barang Kosong', 'Settings']) {
    expect(screen.getByRole('button', { name: label })).toBeInTheDocument();
  }
  const navigation = screen.getByRole('navigation', { name: 'Modul CH Ultimate' });
  expect(navigation.querySelectorAll('.nav-glyph')).toHaveLength(9);
  expect(navigation.querySelectorAll('.nav-glyph svg')).toHaveLength(9);
  const navigationLabels = Array.from(navigation.querySelectorAll('button')).map((button) => button.getAttribute('aria-label'));
  expect(navigationLabels.slice(0, 2)).toEqual(['SKU Gudang', 'Perubahan SKU']);
