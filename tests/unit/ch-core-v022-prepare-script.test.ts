...
const scriptPath = path.join(
  process.cwd(),
  'scripts/ch-core-v022-prepare.sh',
);
...
describe('CH Core v0.2.2 NAS source preparation', () => {
  ...
  it('locks the exact release archive, target, and complete migration chain', async () => {
    ...
    expect(script).toContain(
      '/volume1/docker/ch-ultimate-dc76d3c0529233974f0d1ec18420a230d0c768a5',
    );
  ...
  it('matches every embedded migration checksum to the reviewed SQL bytes', async () => {
    for (const [relativeScript, expectedCount] of [
      ['scripts/ch-core-v022-preflight.sh', 9],
      ['scripts/ch-core-v022-prepare.sh', 10],
    ] as const) {
