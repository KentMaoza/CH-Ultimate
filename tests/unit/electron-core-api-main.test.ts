...
describe('CH Core endpoint configuration', () => {
  it('accepts only a fixed HTTPS IP literal on port 8443', () => {
    ...
    for (const endpoint of [
      ...
      'https://192.0.2.10',
      'https://user:pass@192.0.2.10:8443',
      'https://192.0.2.10:8443/v1',
      'https://192.0.2.10:8443?mode=test',
      'https://192.0.2.10:8443#core',
    ]) {
      expect(() =>
...
describe('CH Core operation allowlist', () => {
  ...
  it('rejects origin, traversal, control characters, and unknown routes before networking', async () => {
    ...
    const paths = [
      'https://192.0.2.10:8443/v1/bootstrap',
      '//192.0.2.10:8443/v1/bootstrap',
      '/v1/../bootstrap',
      '/v1/%2e%2e/bootstrap',
      '/v1/bootstrap\nX-Test: yes',
      '/v1/not-a-route',
