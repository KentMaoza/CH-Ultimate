...
describe('GitHub pilot release workflow', () => {
  it('gates both platform builds and stages an unpublished release draft', async () => {
    ...
    expect(workflow).toContain('default: pilot-v0.2.7-r2');
  ...
  it('creates r2 only after exhaustive release and exact Git-tag checks pass', () => {
    ...
    stagePilotDraft({
      ...
      commitSha,
      releaseTag: 'pilot-v0.2.7-r2',
      fileExists: () => true,
      runGh: (args) => {
        calls.push(args);
        if (args.includes('--slurp')) {
          return JSON.stringify([[{ tag_name: 'pilot-v0.2.7' }], []]);
        }
    ...
    expect(calls[1]).toEqual([
      'api',
      'repos/KentMaoza/CH-Ultimate/git/matching-refs/tags/pilot-v0.2.7-r2',
    ]);
    ...
    expect(calls[2]).toEqual([
      ...
      commitSha,
      '--title',
      'CH Ultimate pilot v0.2.7 r2',
      '--notes-file',
      'docs/releases/pilot-0.2.7.md',
    ]);
  });
      ...
      runGh: (args) => {
        ...
        if (args.includes('--slurp')) {
          return JSON.stringify([
            [{ tag_name: 'pilot-v0.2.7' }],
            [{ tag_name: 'pilot-v0.2.7-r2' }],
          ]);
        }
        if (args.some((arg) => arg.includes('matching-refs'))) return '[]';
        return '';
      },
    })).toThrow('Release pilot-v0.2.7-r2 already exists.');
    ...
    expect(() => stagePilotDraft({
      ...
      runGh: (args) => {
        ...
        if (args.some((arg) => arg.includes('matching-refs'))) {
          return JSON.stringify([{
            ref: 'refs/tags/pilot-v0.2.7-r2',
            object: { sha: '23dea103864a47925c2d7da06dfc69ef380ceba6' },
          }]);
        }
        return '';
      },
    })).toThrow('Tag pilot-v0.2.7-r2 already exists.');
    expect(calls).toEqual([
      [
        'api', '--paginate', '--slurp',
        'repos/KentMaoza/CH-Ultimate/releases?per_page=100',
  ...
  it('can recover from an incomplete r2 only by creating a fresh r3 candidate', () => {
      ...
      runGh: (args) => {
        ...
        if (args.includes('--slurp')) {
          return JSON.stringify([[{
            ...
            assets: [{ name: 'CH-Ultimate-0.2.7-Setup.exe', size: 149_000_000 }],
          }]]);
        }
        if (args.some((arg) => arg.includes('matching-refs'))) return '[]';
        return '';
    ...
    expect(calls[1]).toContain(
      'repos/KentMaoza/CH-Ultimate/git/matching-refs/tags/pilot-v0.2.7-r3',
    );
    expect(calls[2]?.slice(0, 3)).toEqual(['release', 'create', 'pilot-v0.2.7-r3']);
      ...
      runGh: (args) => {
        ...
        if (args.includes('--slurp')) {
          return JSON.stringify([[{
            ...
            assets: [
              { name: 'CH-Ultimate-0.2.7-Setup.exe', size: 149_000_000 },
              { name: 'CHU-Companion-Mobile-0.2.7-release.apk', size: 43_000_000 },
  ...
  it('refuses recovery when the immediately previous candidate is already complete', () => {
    ...
    })).toThrow('Previous candidate pilot-v0.2.7-r2 is not an eligible predecessor draft.');
  ...
  it.each([
    ['duplicate', [
      {
        tag_name: 'pilot-v0.2.7-r2', draft: true, prerelease: true,
    ...
    ['missing or gapped', []],
    ['published', [{
      tag_name: 'pilot-v0.2.7-r2', draft: false, prerelease: true,
      target_commitish: '141961c4a2ef58cecd6525c88903f76d929367b5', assets: [],
    }]],
    ['not prerelease', [{
      tag_name: 'pilot-v0.2.7-r2', draft: true, prerelease: false,
      target_commitish: '141961c4a2ef58cecd6525c88903f76d929367b5', assets: [],
    }]],
    ['wrong target', [{
      tag_name: 'pilot-v0.2.7-r2', draft: true, prerelease: true,
    ...
    ['malformed assets', [{
      tag_name: 'pilot-v0.2.7-r2', draft: true, prerelease: true,
      target_commitish: '141961c4a2ef58cecd6525c88903f76d929367b5',
      assets: [{ name: 'CH-Ultimate-0.2.7-Setup.exe', size: 'large' }],
    }]],
  ...
  it('keeps the v0.2.7 real-use release contract aligned', async () => {
    const [
      workflow,
      packageManifest,
      packageLock,
    ...
    expect(androidBuild).toContain('versionCode 14');
    ...
    expect(releaseNotes).toMatch(/deploy Core.+commit rilis v0\.2\.5/is);
